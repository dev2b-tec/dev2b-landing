...
export default function Footer() {
  return (
    <footer className="bg-[var(--d2b-bg-deep)] text-[var(--d2b-text-muted)] border-t border-[var(--d2b-border-mid)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-16 pb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-5">
              ...
              <span className="text-[var(--d2b-text-primary)] font-bold text-xl tracking-tight">
                DEV<span className="text-[#7C4DFF]">2B</span>
              </span>
            </div>
            <p className="text-sm leading-relaxed mb-6 text-pretty text-[var(--d2b-text-muted)]">
              A plataforma completa de gestao digital para negocios de servicos que querem crescer com tecnologia.
            </p>
            <div className="flex items-center gap-2.5">
              <a href="https://instagram.com" target="_blank" rel="noreferrer" aria-label="Instagram da DEV2B" className="w-9 h-9 rounded-lg bg-[var(--d2b-border)] flex items-center justify-center hover:bg-[#7C4DFF]/30 hover:text-[#C084FC] text-[var(--d2b-text-muted)] transition-colors">
              ...
              <a href="https://linkedin.com" target="_blank" rel="noreferrer" aria-label="LinkedIn da DEV2B" className="w-9 h-9 rounded-lg bg-[var(--d2b-border)] flex items-center justify-center hover:bg-[#7C4DFF]/30 hover:text-[#C084FC] text-[var(--d2b-text-muted)] transition-colors">
              ...
              <a href="https://youtube.com" target="_blank" rel="noreferrer" aria-label="YouTube da DEV2B" className="w-9 h-9 rounded-lg bg-[var(--d2b-border)] flex items-center justify-center hover:bg-[#7C4DFF]/30 hover:text-[#C084FC] text-[var(--d2b-text-muted)] transition-colors">
              ...
              <a href="https://facebook.com" target="_blank" rel="noreferrer" aria-label="Facebook da DEV2B" className="w-9 h-9 rounded-lg bg-[var(--d2b-border)] flex items-center justify-center hover:bg-[#7C4DFF]/30 hover:text-[#C084FC] text-[var(--d2b-text-muted)] transition-colors">
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <h3 className="text-[var(--d2b-text-secondary)] font-semibold text-xs mb-5 uppercase tracking-widest">{category}</h3>
              <ul className="flex flex-col gap-2.5">
                {links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-sm text-[var(--d2b-text-muted)] hover:text-[var(--d2b-brand-light)] transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        {/* Bottom bar */}
          <div className="pt-8 border-t border-[var(--d2b-border)] flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-xs text-[var(--d2b-text-faint)]">
            &copy; {new Date().getFullYear()} DEV2B. Todos os direitos reservados.
          </p>
          <div className="flex items-center gap-6">
            <Link href="/privacidade" className="text-xs text-[var(--d2b-text-faint)] hover:text-[var(--d2b-text-secondary)] transition-colors">Politica de Privacidade</Link>
            <Link href="/termos" className="text-xs text-[var(--d2b-text-faint)] hover:text-[var(--d2b-text-secondary)] transition-colors">Termos de Uso</Link>
            <Link href="/lgpd" className="text-xs text-[var(--d2b-text-faint)] hover:text-[var(--d2b-text-secondary)] transition-colors">LGPD</Link>
          </div>
        </div>
      </div>
    </footer>

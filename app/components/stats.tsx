...
export default function Stats() {
  return (
    <section className="py-20 lg:py-24 bg-[var(--d2b-bg-main)] relative overflow-hidden border-y border-[var(--d2b-border-mid)]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        <p className="text-center text-[var(--d2b-text-muted)] text-sm font-semibold uppercase tracking-widest mb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 text-center">
          {stats.map((stat) => (
            <div key={stat.label} className="flex flex-col items-center gap-2">
              <span className="text-3xl lg:text-4xl font-black text-[var(--d2b-text-primary)]">{stat.value}</span>
              <span className="text-[var(--d2b-text-muted)] text-sm">{stat.label}</span>
            </div>
          ))}
        </div>

        <div className="mt-14 text-center">
          <p className="text-[var(--d2b-text-muted)] text-base mb-6">
            Mais de 40 mil negocios tornaram sua gestao mais agil.{" "}
            <strong className="text-[var(--d2b-text-secondary)]">Voce ta esperando o que?</strong>
          </p>
          <Link
            href={REGISTER_URL}
            target="_blank"

      ...
      <section className="py-16 lg:py-20 bg-[var(--d2b-bg-main)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col gap-8">
            {casos.map((caso, idx) => (
              <div key={caso.name} className={`flex flex-col ${idx % 2 === 0 ? "lg:flex-row" : "lg:flex-row-reverse"} rounded-2xl overflow-hidden border border-[var(--d2b-border-mid)] bg-[var(--d2b-bg-surface)] hover:border-[#7C4DFF]/30 transition-all group`}>
                ...
                <div className="flex-1 p-8 lg:p-10 flex flex-col justify-center">
                  ...
                  <p className="text-[var(--d2b-text-secondary)] text-lg leading-relaxed mb-6 italic text-pretty">&ldquo;{caso.depoimento}&rdquo;</p>
                  <div className="flex items-center gap-3">
                    <Image src={caso.avatar} alt={caso.name} width={44} height={44} className="rounded-full object-cover" />
                    <div>
                      <p className="font-bold text-[var(--d2b-text-primary)]">{caso.name}</p>
                      <p className="text-[var(--d2b-text-muted)] text-sm">{caso.negocio} &bull; {caso.segmento}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-14 text-center">
            <Link href={REGISTER_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl bg-[#7C4DFF] text-white font-bold hover:bg-[#6B3FE8] transition-all shadow-xl shadow-[#7C4DFF]/40 hover:-translate-y-0.5">

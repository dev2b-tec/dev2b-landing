...
export default function TreinamentosPage() {
  return (
    <main>
      <Navbar />
      <section className="relative pt-32 pb-20 bg-[#0D0520] min-h-screen overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-14">
            {trilhas.map(({ icon: Icon, title, desc, tag, count }) => (
              <div key={title} className="p-7 rounded-2xl border border-[#7C4DFF]/12 bg-[#120328]/60 hover:border-[#7C4DFF]/35 transition-all group">
          ...
          <div className="max-w-2xl mx-auto text-center p-10 rounded-2xl border border-[#7C4DFF]/20 bg-[#7C4DFF]/8">
            <h2 className="text-2xl font-black text-white mb-3">Proximo webinar ao vivo</h2>
            <p className="text-white/50 mb-2">Como aumentar seu faturamento com clube de assinaturas</p>
            <p className="text-[#C084FC] font-semibold text-sm mb-6">Quarta-feira, 01 de Abril &bull; 19h (horario de Brasilia)</p>
            <Link href="/planos" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl bg-[#7C4DFF] text-white font-bold hover:bg-[#6B3FE8] transition-colors shadow-lg shadow-[#7C4DFF]/30">
